...
public class ProductAggregateService implements AbstractProductController, AbstractRecommendationController, AbstractReviewController {
    ...
    private ProductAggregate createAggregateProduct(Product product, List<Review> reviews, List<Recommendation> recommendations) {
        ...
        String reviewAddress = !reviews.isEmpty() ? reviews.getFirst().serviceAddress() : "";
        String recommendationAddress = !recommendations.isEmpty() ? recommendations.getFirst().serviceAddress() : "";
        ServiceAddresses serviceAddresses = new ServiceAddresses(addressUtil.getServiceAddress(), product.serviceAddress(), recommendationAddress, reviewAddress);
